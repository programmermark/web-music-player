<template>
  <div class="playlist-detail pl-[30px] pr-[30px] w-full box-sizing-border-box">
    <!-- 歌单信息 -->
...
import type { IPlaylistInfo } from "./interface/playDetail";

const route = useRoute();

/** 歌单信息 */
const playlistInfo = ref<IPlaylistInfo | undefined>(undefined);
/** 歌单歌曲列表 */
const songList = ref<ISongDetail[]>([]);
/** 歌曲订阅用户列表 */
const subscribers = ref<IPlayListSubscriber[]>([]);
...
const getPlaylistDetail = async (id: number) => {
  ...
  if (playlist.subscribers && playlist.subscribers.length > 0) {
    subscribers.value = playlist.subscribers.map((subscriber) => ({
      id: subscriber.id,
