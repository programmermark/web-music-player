...
/** 当前视频类别 */
const currentCategory = computed(() => store.state.videolist.currentCategory);
...
const handleCategoryChange = (category?: IVideoCategory) => {
  if (category) {
    store.commit("videolist/setCurrentCategory", category);
  } else {
    store.commit("videolist/setCurrentCategory", null);
  }
};
