...
import type { IPlayerState, ISongState, IUsePlayerState } from "../interface";
  ...
  const initAudio = () => {
    ...
    ele.onprogress = () => {
      try {
        if (ele.buffered.length > 0) {
          playerStore.currentDuration = Number(ele.duration.toFixed(3));
        }
      } catch (error) {
    ...
    ele.ontimeupdate = () => {
      if (!songState.isAdjusting) {
        _.debounce(() => {
          playerStore.currentTime = Number(ele.currentTime.toFixed(3));
        }, 1000)();
